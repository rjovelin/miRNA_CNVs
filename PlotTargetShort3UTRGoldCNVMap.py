...
#
#
## make a dictionary {domain: {gene: [targets, seq_length, normalized_targets, CNV status]}}
## for genes with short 3'UTR
#regions = ['5UTR', 'CDS']
#targetscan, miranda = {}, {} 
#predictors = ['targetscan', 'miranda']
#
## get file with UTR status (short, long)
#UTR_file = 'H_sapiens_3UTR_length_valid_chromos.txt'
#print(UTR_file)
...
#        predicted_targets = 'H_sapiens_' + domain + '_valid_chromos_predicted_sites_' + predictors[i] + '.txt'
...
#                    targetscan[domain][gene] = copy.deepcopy(targets[gene])
#        elif i == 1:
#            miranda[domain] = {}
#            for gene in targets:
#                # record gene with short 3'UTR
#                if gene in UTR_length and UTR_length[gene] == 'short':
#                    miranda[domain][gene] = copy.deepcopy(targets[gene])
#
## check that all domains have been recorded for miranda and targetscan
#assert len(targetscan) == len(miranda) == 2, 'different number of domains depending on predictor'
#
## remove genes if genes do not have CNV status
#for region in targetscan:
#    to_remove, to_keep = [], []
#    for gene in targetscan[region]:
#        if len(targetscan[region][gene]) != 4:
#            assert len(targetscan[region][gene]) == 3, 'gene without CNV status should have correct target information'
#            assert targetscan[region][gene][-1] not in ['CNV', 'not_CNV'], 'gene without CNV status should have correct target information'
#            to_remove.append(gene)
#        else:
#            to_keep.append(gene)
...
#        if len(miranda[region][gene]) != 4:
#            assert len(miranda[region][gene]) == 3, 'gene without CNV status should have correct target information'
#            assert miranda[region][gene][-1] not in ['CNV', 'not_CNV'], 'gene without CNV status should have correct target information'
#            to_remove.append(gene)
#        else:
#            to_keep.append(gene)
#    print('remove {0} genes from {1}'.format(len(to_remove), region))  
...
#for domain in targetscan:
#    for gene in targetscan[domain]:
#        assert len(targetscan[domain][gene]) == 4, 'gene in targetscan does not have all required values'
#    for gene  in miranda[domain]:
#        assert len(miranda[domain][gene]) == 4, 'gene in miranda does not have all required values'
#
...
## {predictor: [[5UTR targets cnv], [5UTR targets noncnv], [CDS targets cnv], [CDS targets non-cnv]]
#AllData = {}
#AllData['targetscan'], AllData['miranda'] = [[], [], [], []], [[], [], [], []]
#for region in targetscan:
#    for gene in targetscan[region]:
#        if region == '5UTR' and targetscan[region][gene][-1] == 'CNV':
#            i = 0
...
#for region in miranda:
#    for gene in miranda[region]:
#        if region == '5UTR' and miranda[region][gene][-1] == 'CNV':
#            i = 0
